...
namespace io
{
	...
	*/
	...
	/*!
        	...
        	\sa afficher()
	*/
	template<typename T> void liste_elements(vector<T> vect_element, bool need_desc)
	{
	...
	*/
	template<typename T> T choix_unique_element(vector<T> vect_element, bool need_desc)
	{
        	string type_name = typeid(T).name();                            //String à partir du type appelant
        	type_name = type_name.substr(2, type_name.length());            //Conservation des caractères pertinents

        	cout << "Veuillez choisir votre " << type_name << " (0-9): ";
